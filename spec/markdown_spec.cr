...
describe Fluence::Markdown do
  it "test internal links" do
    ...
    Fluence::Markdown.to_markdown("[[test]]", page, index).
      should eq("[test](/pages/test)")
    Fluence::Markdown.to_markdown("[not](http://itisnot/not)", page, index).
      should eq("[not](http://itisnot/not)")
    Fluence::Markdown.to_markdown("[\\[page]]", page, index).
      should eq("[\\[page]]")
  end
  ...
  it "test internal link with fixed title" do
    page = Fluence::Page.new("test")
    index = Fluence::Page::Index.new("")
    Fluence::Markdown.to_markdown("[[test|title]]", page, index).
      should eq("[title](/pages/test)")
    Fluence::Markdown.to_markdown("[[test-longer|title a bit longer]]", page, index).
      should eq("[title a bit longer](/pages/test-longer)")
    Fluence::Markdown.to_markdown("[[test-empty|]]", page, index).
